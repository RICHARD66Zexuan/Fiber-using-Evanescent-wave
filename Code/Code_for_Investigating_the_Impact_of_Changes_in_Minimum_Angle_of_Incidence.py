...
# ----------------------------------------------------------------------------
anglecc=np.arange(3,87.5,1)
minangle_content=[math.radians(i) for i in anglecc]
c=len(anglecc)
i=0

for j in range(c):  
    minangle=minangle_content[j]
    maxangle=1.569
    fiber_critical_angle=f.get_fiber_critical_angle(fiber_index[i], medium_refraction_index[i])
    n=f.calculate_angle_divided_number(minangle, maxangle, increament)
    incident_energy[i]=f1.chageincident_energy_simple(incident_energy[i], n)
    ...
    plt.title(fr"$\theta_m$ = {int(anglecc[j])}°", fontsize=10, fontproperties=font_properties)
